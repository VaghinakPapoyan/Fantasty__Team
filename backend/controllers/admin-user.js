...
export async function getAllUsersNames(req, res) {
  try {
    const users = await User.find({}, "_id email"); // Only select `_id` and `fullname`
...
export async function readNotification(req, res) {
  ...
  try {
    const updatedUser = await User.findOneAndUpdate(
      { _id: userId }, // Match userId and notificationId
      { $set: { "notifications.$[].active": false } }, // Set the active status to false
    ...
    return res.status(200).send(updatedUser.notifications);
  } catch (error) {
...
export async function sendNotificationsToUsers(req, res) {
  try {
    ...
    // Update notifications for the given user IDs
    const result = await User.updateMany(
      { _id: { $in: userIds } }, // Match the provided user IDs
      { $push: { notifications: notification } } // Add the notification to their notifications array
    );

    console.log(`Notification sent to ${result.modifiedCount} users.`);
    return res
      .status(200)
      .send({ message: `Notification sent to ${result.modifiedCount} users.` });
  } catch (error) {
  ...
  try {
    ...
    const user = await User.findOneAndUpdate(
      { _id: userId },
      {
        $push: {
          notifications: {
            text: message, // Add the notification text
            active: true, // Default active value
          },
        },
      }
    );
